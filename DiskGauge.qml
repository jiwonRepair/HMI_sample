...
Item {
    ...
    property var thresholds: [
        ...
        { threshold: 10, color: "#f1c40f" },  // 노란색 (주의)
    ...
    Canvas {
        ...
        onPaint: {
            ...
            ctx.fillText("DISK", centerX, centerY + 12)
        }
    }
    ...
    Connections {
        ...
        function onDiskUsageChanged() {
            //console.log("[QML] disk Usage Updated:", diskMonitor.diskUsage);
            memGauge.value = diskMonitor.diskUsage;  // 강제 업데이트

            // 🔥 canvas가 존재하는 경우에만 requestPaint 실행
            if (memGauge.canvas) {
                memGauge.canvas.requestPaint();
            } else {
                console.warn("[QML] diskGauge.canvas is undefined!");
            }
        }
    }
